...
export class GameScene extends Phaser.Scene {
  ...
  private setupSocketListeners(): void {
    ...
    // AI Office Agents: server-driven speech bubbles ({npcId, text, ttl})
    this.socket.on("npc:speech", (data: { npcId: string; text: string; ttl?: number }) => {
      const npc = this.npcSprites.find(n => n.id === data.npcId);
      if (!npc || !data.text) return;
      this.showNpcBubbleIcon(npc.id, data.text);
      const bubble = this.npcBubbles.get(npc.id);
      if (!bubble) return;
      const ttl = Math.max(1000, Math.min(10000, data.ttl ?? 3000));
      this.tweens.add({
        targets: bubble,
        alpha: 0,
        duration: 400,
        delay: Math.max(0, ttl - 400),
        onComplete: () => {
          bubble.destroy();
          if (this.npcBubbles.get(npc.id) === bubble) {
            this.npcBubbles.delete(npc.id);
          }
        },
      });
    });
